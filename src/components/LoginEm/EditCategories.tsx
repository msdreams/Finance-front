...
import { ExpenseDeleteCategory, ExpenseGetAllCategories, IncomeDeleteCategory, IncomeGetAllCategories } from "../../features/expenseIncomeCategorySlice";
...
import { ModalApruve } from "../ModalApruve";
...
export const EditCategories = () => {
  ...
  const { isOpen: isDeleteExpenseOpen, onOpen: onOpenExpense, onOpenChange: onOpenDeleteExpense } = useDisclosure();
  const { isOpen: isDeleteIncomeOpen, onOpen: onOpenIncome, onOpenChange: onOpenDeleteIncome } = useDisclosure();
  ...
  const handleExpenseDelete = () => {
    if (category) {
      dispatch(ExpenseDeleteCategory(`${category.id}`))
  ...
  const handleIncomeDelete = () => {
    if (category) {
      dispatch(IncomeDeleteCategory(`${category.id}`))
        ...
        <div className="flex flex-col gap-6 md:gap-4 md:flex-row w-full">
          <div className="flex w-full flex-col gap-2 ">
            <div className="flex flex-row md:flex-col gap-4 items-baseline pb-1 ">
              ...
              <Tooltip className="font-sans" content="Create New Expense">
                <Button 
                  ...
                  isLoading={isLoading} 
                  onPress={() => { onOpen(); setCategoryType("Expense") }}
                >
                    +
                </Button>
              </Tooltip>
            ...
            <div className="flex w-full max-h-[120px] md:max-h-[500px] overflow-y-scroll dark-scroll flex-col border border-gray-500 rounded-lg ">
              {expenseCategoryAll.filter(c => c.id !== 6).map(c => (
                <div
                  key={c.id}
                ...
                >
                  ...
                  <div className={`flex flex-row gap-2 transition-all duration-300 
                    ${category === c ? "opacity-100 max-h-screen" : "opacity-0 max-h-0 overflow-hidden"}`}>
                    <Tooltip 
                      color="danger" 
          ...
          <div className="flex w-full flex-col gap-2 ">
            <div className="flex flex-row md:flex-col gap-4 items-baseline pb-1 ">
              ...
              <Tooltip className="font-sans" content="Create New Income">
                <Button 
                  ...
                  isLoading={isLoading} 
                  onPress={() => { onOpen(); setCategoryType("Income") }}
                >
                    +
                </Button>
              </Tooltip>
            ...
            <div className="flex w-ful max-h-[120px] md:max-h-[500px] overflow-y-scroll dark-scroll flex-col border border-gray-500 rounded-lg">
              {incomeCategoryAll.filter(c => c.id !== 6).map(c => (
                <div
                  key={c.id}
                  className={`flex flex-row justify-between items-center p-2 pr-4 pl-4 
                    ${category === c ? " bg-gray-500 " : "bg-gray-600"}
