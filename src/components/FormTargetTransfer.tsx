...
export const FormTargetTransfer: React.FC<Props> = ({ selectedAccount }) => {
  ...
  useEffect(() => {
    ...
    dispatch(GetAllTargets())
      ...
      .then((r) => {
        if (r && r.length > 0)
          setSelectedKeys(new Set([`0-${r[0].name}-${r[0].id}`]));
      });
  }, [dispatch]);
    ...
    <>
      {targets.length > 0 && (
        ...
        >
          ...
          {errowMessage && (
            <div className="text-danger-300">{errowMessage}</div>
          )}
          ...
          <div className="flex flex-col md:flex-row md:items-baseline w-full gap-2">
            <Input
              ...
              name="amount"
              placeholder={`Expected Sum: ${
                targets[+CurrentTarget.split("-")[0]].downPayment || ""
              } $`}
              type="number"
              min="1"
            />
